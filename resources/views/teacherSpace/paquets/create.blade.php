						...
						<form action="{{ route('paquets.store') }}" method="POST">
							...
							<div class="form-group {{ $errors->has('groupe_id') ? 'has-danger' : '' }}">
								...
								<select name="groupe_id" class="chosen-select" style="width: 100%">
									@foreach($groupes as $groupe)
										<option value="{{ $groupe->id }}">{{ $groupe->specialite }} {{ $groupe->annee }} G{{ $groupe->numero
											}}
										</option>
									@endforeach
								</select>
							...
							<div class="form-group {{ $errors->has('date_limite_encryptor') ? 'has-danger' : '' }}">
								...
								<input type="date" name="date_limite_encryptor" id="date_limite_encryptor" class="form-control" required
											 value="{{ old('date_limite_encryptor') }}">
							...
							<div class="row">
								...
								<div class="form-group {{ $errors->has('date_limite_correcteur1') ? 'has-danger' : '' }} col-md-6">
									...
									<input type="date" name="date_limite_correcteur1" id="date_limite_correcteur1" class="form-control"
												 required="required" value="{{ old('date_limite_correcteur1') }}">
							...
							<div class="row">
								...
								<div class="form-group {{ $errors->has('date_limite_correcteur2') ? 'has-danger' : '' }} col-md-6">
									...
									<input type="date" name="date_limite_correcteur2" id="date_limite_correcteur2" class="form-control"
												 value="{{ old('date_limite_correcteur2') }}">
							...
							<div class="row">
								...
								<div class="form-group {{ $errors->has('date_limite_correcteur3') ? 'has-danger' : '' }} col-md-6">
									...
									<input type="date" name="date_limite_correcteur3" id="date_limite_correcteur3" class="form-control"
												 value="{{ old('date_limite_correcteur3') }}">
