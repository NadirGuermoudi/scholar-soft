@extends('layouts.master', ['title' => 'Modifier un paquet'])
...
<div class="card-block">
	...
	<h4 class="card-title">Modifier le paquet {{ $paquet->type }} | {{ $paquet->module }}</h4>
	
	<form action="{{ route('paquets.update', $paquet) }}" method="POST">
		@method('PUT')
		@csrf
		...
		<div class="form-group {{ $errors->has('type') ? 'has-danger' : '' }}">
			...
			<select name="type" class="chosen-select" style="width: 100%">
				<option value="EXAM" {{ ($paquet->type == 'EXAM') ? 'selected' : '' }}>Examen</option>
				<option value="CC" {{ ($paquet->type == 'CC') ? 'selected' : '' }}>Control continu</option>
			</select>
		...
		<div class="form-group {{ $errors->has('module') ? 'has-danger' : '' }}">
			...
			<input type="text" name="module" id="module" class="form-control" required="required" value="{{ $paquet->module }}">
		...
		<div class="row">
			<div class="form-group {{ $errors->has('type_calcul') ? 'has-danger' : '' }} col-md-6">
				...
				<select name="type_calcul" class="chosen-select" style="width: 100%">
					<option value="MAX" {{ ($paquet->type_calcul == 'MAX') ? 'selected' : '' }}>Maximum(corr1, corr2)</option>
					<option value="MOY" {{ ($paquet->type_calcul == 'MOY') ? 'selected' : '' }}>Moyenne(corr1, corr2)</option>
				</select>
			...
			<div class="form-group {{ $errors->has('difference') ? 'has-danger' : '' }} col-md-6">
				...
				<input type="number" name="difference" id="difference" class="form-control" required value="{{ $paquet->difference }}">
		...
		<div class="form-group {{ $errors->has('date_limite_encryptor') ? 'has-danger' : '' }}">
			...
			<input type="date" name="date_limite_encryptor" id="date_limite_encryptor" class="form-control" required value="{{ old('date_limite_encryptor') ?? $paquet->date_limite_encryptor }}">
		...
		<div class="row">
			<div class="form-group {{ $errors->has('correcteur1_id') ? 'has-danger' : '' }} col-md-6">
				...
				<select name="correcteur1_id" class="chosen-select" style="width: 100%">
					...
					<option value="{{ $enseignant->id }}" {{ ($paquet->correcteur1_id == $enseignant->id) ? 'selected' : '' }}>{{ $enseignant->fullName }}</option>
					@endforeach
				</select>
			...
			<div class="form-group {{ $errors->has('date_limite_correcteur1') ? 'has-danger' : '' }} col-md-6">
				...
				<input type="date" name="date_limite_correcteur1" id="date_limite_correcteur1" class="form-control" required="required" value="{{ old('date_limite_correcteur1') ?? $paquet->date_limite_correcteur1 }}">
		...
		<div class="row">
			<div class="form-group {{ $errors->has('correcteur2_id') ? 'has-danger' : '' }} col-md-6">
				...
				<select name="correcteur2_id" class="chosen-select" style="width: 100%">
					...
					@foreach($enseignants as $enseignant)
					<option value="{{ $enseignant->id }}" {{ ($paquet->correcteur2_id == $enseignant->id) ? 'selected' : '' }}>{{ $enseignant->fullName }}</option>
					@endforeach
				</select>
			...
			<div class="form-group {{ $errors->has('date_limite_correcteur2') ? 'has-danger' : '' }} col-md-6">
				...
				<input type="date" name="date_limite_correcteur2" id="date_limite_correcteur2" class="form-control" value="{{ old('date_limite_correcteur2') ?? $paquet->date_limite_correcteur2 }}">
		...
		<div class="row">
			<div class="form-group {{ $errors->has('correcteur3_id') ? 'has-danger' : '' }} col-md-6">
				...
				<select name="correcteur3_id" class="chosen-select" style="width: 100%">
					<option value="0">Aucun</option>
					@foreach($enseignants as $enseignant)
					<option value="{{ $enseignant->id }}" {{ ($paquet->correcteur3_id == $enseignant->id) ? 'selected' : '' }}>{{ $enseignant->fullName }}</option>
					@endforeach
				</select>
			...
			<div class="form-group {{ $errors->has('date_limite_correcteur3') ? 'has-danger' : '' }} col-md-6">
				...
				<input type="date" name="date_limite_correcteur3" id="date_limite_correcteur3" class="form-control" value="{{ old('date_limite_correcteur3') ?? $paquet->date_limite_correcteur3 }}">
		...
		<div class="form-group {{ $errors->has('responsable_rendu') ? 'has-danger' : '' }}">
			<input type="checkbox" name="responsable_rendu" id="responsable_rendu" value="responsable_rendu" {{ $paquet->responsable_rendu ? 'checked' : '' }}>
		...
		<div class="form-group">
			<button class="btn btn-primary btn-block" type="submit">Modifier le paquet</button>
		</div>
