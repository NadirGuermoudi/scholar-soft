...
<div class="card-block">
	...
	<table id="example23" class="display nowrap table table-hover table-striped table-bordered" style="width: 100%;">
		...
		<tbody>
			...
			<tr>
				<td>
					@if($paquet->encrypted) <i class="fas fa-check-circle text-success"></i><p hidden>1</p> @else <i class="fas fa-sync-alt text-warning"></i><p hidden>2</p> @endif
				...
				<td>
					...
					<a href="{{ route('paquets.correct.one', ['paquet' => $paquet, 'correcteur' => 1]) }}" class="btn btn-outline-danger"><i class="fa fa-pen"></i></a>
				</td>
			</tr>
			@endif
			...
			<tr>
				<td>
					@if($paquet->correcteur1_rendu) <i class="fas fa-check-circle text-success"></i><p hidden>1</p> @else <i class="fas fa-sync-alt text-warning"></i><p hidden>2</p> @endif
					{{ $paquet->type }} | {{ $paquet->module }}
				</td>
				<td>{{ $paquet->type_calcul }}-{{ $paquet->difference }}</td>
				<td>{{ $paquet->responsable->fullName }}</td>
				...
				<td>
					...
					<a href="{{ route('paquets.correct.one', ['paquet' => $paquet, 'correcteur' => 2]) }}" class="btn btn-outline-danger"><i class="fa fa-pen"></i></a>
				</td>
			</tr>
			@endif
			...
			<tr>
				<td>
					@if($paquet->correcteur2_rendu) <i class="fas fa-check-circle text-success"></i><p hidden>1</p> @else <i class="fas fa-sync-alt text-warning"></i><p hidden>2</p> @endif
					{{ $paquet->type }} | {{ $paquet->module }}
				</td>
				<td>{{ $paquet->type_calcul }}-{{ $paquet->difference }}</td>
				<td>{{ $paquet->responsable->fullName }}</td>
				...
				<td>
					...
					<a href="{{ route('paquets.correct.one', ['paquet' => $paquet, 'correcteur' => 3]) }}" class="btn btn-outline-danger"><i class="fa fa-pen"></i></a>
				</td>
			</tr>
			@endif
			@endforeach
