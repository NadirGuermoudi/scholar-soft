					...
					<div class="card-block">
						...
						<form action="{{ route('seances.update', $seance) }}" method="POST">
							...
							<div class="form-group {{ $errors->has('type') ? 'has-danger' : '' }}">
								...
								{!! $errors->first('type', '<span class="help-block form-control-feedback">:message</span>') !!}
							</div>
							...
							<div class="form-group {{ $errors->has('groupes_ids[]') ? 'has-danger' : '' }}">
								...
								<select name="groupes_ids[]" class="chosen-select" style="width: 100%" multiple>
									@foreach($groupes as $groupe)
										<option
											value="{{ $groupe->id }}" {{ $seance->groupes()->find($groupe->id) != null ? 'selected' : '' }}>{{
											$groupe->specialite }} {{ $groupe->annee }} G{{ $groupe->numero }}
										</option>
									@endforeach
								</select>
							...
							<div class="form-group {{ $errors->has('heur_debut') ? 'has-danger' : '' }}">
								...
								<input type="time" name="heur_debut" id="heur_debut" class="form-control" required="required"
											 value="{{ substr ($seance->heur_debut, 0, 5) }}">
							...
							<div class="form-group {{ $errors->has('heur_fin') ? 'has-danger' : '' }}">
								...
								<input type="time" name="heur_fin" id="heur_fin" class="form-control" required="required"
											 value="{{ substr ($seance->heur_fin, 0, 5) }}">
							...
							<div class="form-group {{ $errors->has('once_two_week') ? 'has-danger' : '' }}">
								<input type="checkbox" name="once_two_week" id="once_two_week"
											 value="once_two_week" {{ $seance->once_two_week == true ? 'checked' : '' }}>
